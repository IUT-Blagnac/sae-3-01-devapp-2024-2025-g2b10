...
^|Client_Commande.2 ^|Saisir l'adresse de livraison, le mode de livraison, de paiement et valider ^|La commande est enregistrée^|Etre sur le récapitulatif de commande^| OK
^|Client_Commande.3 ^|Cliquer sur 'Mes commandes' ^| Affiche les commandes passées ^|Etre sur l'espace Client (icône 'Compte')^| OK
|====
